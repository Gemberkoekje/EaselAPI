...
export. [`reports.txt`](reports.txt) is every block `easel run` printed after a pass, as
...
What follows is the painter's own `NOTES.md`, verbatim but for its headings, which are
set here as bold lines, and where marked.
...
> | Lightest | the head's top plane, as planned (the plan's measure reads it at 0.52 because the eye sits inside the place) |
...
> - **Highlights landed dull** because they mixed into the wet shade under them.
>   `s.dry()` before the lights, which the exercises had shown and the painting still had
>   to relearn.
...
- **The verdict's measured claims, re-measured** — in full in `PLAN-0.8.0.md`, section 3.
  ...
  *at 0.52 instead of 0.64 until I added dry()* — the session shows the head's top
  ...
  at the time the painter put the reading down to the eye itself. The feather: `REFERENCE.md`'s
...
- **Found while filing, not by the painter.** *A union of parts came first:* the first
  ...
  under it, at `0.0068`, seven pixels. *What it read*, from the session's transcript: the
