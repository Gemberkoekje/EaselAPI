...
def lay_warm_rim():
    # The side of the figure toward the lamp: without it the shawl is lost against the wall.
    p["warm_rim"] = p.at_value(p.mix("shawl_lit", "glow_wall", 0.5), 0.29)
    s.stroke([P(598, 562), P(634, 602), P(658, 664), P(672, 764), P(682, 880)], "round_soft",
             "warm_rim", size=0.016, opacity=0.7, pressure=[0.5, 1.0, 0.8, 0.4, 0.05],
             clip=shawl)


# The fist has failed twice -- a disc, then a bun -- so it is redrawn in a pass of its own
# (p07); lay_bunch and lay_knuckles are kept for the record and not run.
lay_folds()
lay_rims()
lay_warm_rim()
s.stroke([P(420, 560), P(446, 572), P(462, 590)], "bristle", "flour", size=0.02, load=0.18,
