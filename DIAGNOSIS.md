...
have read a list of ways to fail — which may teach you to see failures you were not
going to make, and is the one thing `LESSONS.md` measured as not working.

**`easel diagnose` is the interface, and it follows the pointer for you.** You have
rehearsed a pass and looked at it; describe what is wrong in your own words — `easel
diagnose concentric rings` — and what comes back is the passage, not the row. `diagnose`
through the MCP server, `easel.diagnosis.answer()` from Python, `--list` for the old grep.

**Do not work from the row.** One session recognised five on sight, followed none, and
repaired a staircase the expensive way while the row's own target held the cheap one.
**A row naming a file you were not given is not for you.**

It indexes [`PAINTER.md`](PAINTER.md), [`PAINTING.md`](PAINTING.md),
[`RECIPES.md`](RECIPES.md), [`REFERENCE.md`](REFERENCE.md) and
[`CALIBRATION.md`](CALIBRATION.md). `tests/test_diagnosis.py` resolves every pointer to
its passage, because a dead one is now a command that answers with an apology.
