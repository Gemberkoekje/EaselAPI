# Pass 9: REHEARSAL ONLY. The first hair pass read as pale blond. Try two
# darker repaints on a copy of the canvas before spending a stroke.
...
SIZES = [0.055, 0.048, 0.062, 0.056, 0.066, 0.052, 0.066, 0.072, 0.072, 0.076,
         0.070, 0.060]


def plan(colour):
    return [{"points": pts, "brush": "flat", "color": colour, "size": sz,
             "load": 1.0, "pressure": "even"}
            for pts, sz in zip(MASS, SIZES)]


print("A mid-brown 0.30 :", s.rehearse(plan("h_a"), reference=REF, region=span("D1", "G5")))
print("B near-floor dark:", s.rehearse(plan("h_b"), reference=REF, region=span("D1", "G5")))
